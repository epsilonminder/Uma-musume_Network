var styles = [ {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "size_rank",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 9,
      "border-width" : 0.0,
      "color" : "rgb(51,51,51)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 12.0,
      "shape" : "rectangle",
      "text-opacity" : 1.0,
      "background-color" : "rgb(204,204,255)",
      "height" : 12.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(76,76,76)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Big Labels",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 24,
      "border-width" : 0.0,
      "color" : "rgb(51,51,51)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 5.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(255,255,255)",
      "height" : 5.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,0,102)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 1.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(183,183,183)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Ripple",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(51,153,255)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 8,
      "border-width" : 20.0,
      "color" : "rgb(19,58,96)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 50.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(255,255,255)",
      "height" : 50.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,204)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 3.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(51,153,255)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Nested Network Style",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 12,
      "border-width" : 2.0,
      "color" : "rgb(0,0,0)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 60.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(255,255,255)",
      "height" : 40.0,
      "content" : "data(shared_name)"
    }
  }, {
    "selector" : "node[has_nested_network]",
    "css" : {
      "background-color" : "rgb(255,255,255)"
    }
  }, {
    "selector" : "node[has_nested_network]",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[has_nested_network]",
    "css" : {
      "text-valign" : "bottom"
    }
  }, {
    "selector" : "node[has_nested_network]",
    "css" : {
      "color" : "rgb(0,102,204)"
    }
  }, {
    "selector" : "node[has_nested_network]",
    "css" : {
      "border-color" : "rgb(0,102,204)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 1.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(64,64,64)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Marquee",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(255,255,255)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 12,
      "border-width" : 10.0,
      "color" : "rgb(102,102,102)",
      "text-valign" : "bottom",
      "text-halign" : "center",
      "width" : 20.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(0,204,255)",
      "height" : 20.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,0,102)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(255,255,255)",
      "color" : "rgb(102,102,102)",
      "target-arrow-color" : "rgb(255,255,255)",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "triangle",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "dashed",
      "font-size" : 8,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(255,255,255)",
      "content" : "data(interaction)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Solid",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "Dialog",
      "font-weight" : "normal",
      "font-size" : 14,
      "border-width" : 0.0,
      "color" : "rgb(0,0,0)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 40.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(102,102,102)",
      "height" : 40.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 12.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "Dialog",
      "font-weight" : "normal",
      "line-color" : "rgb(204,204,204)",
      "content" : "data(interaction)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "default",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(204,204,204)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 20,
      "border-width" : 0.0,
      "color" : "rgb(0,0,0)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 200.0,
      "shape" : "roundrectangle",
      "text-opacity" : 1.0,
      "background-color" : "rgb(0,255,255)",
      "height" : 35.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node[name = 'ナリタタイシン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'マルゼンスキー']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'キングヘイロー']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ゴールドシップ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'アグネスタキオン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ハルウララ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ビワハヤヒデ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ダイワスカーレット']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'マチカネフクキタル']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ウイニングチケット']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'エアグルーヴ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ウオッカ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'テイエムオペラオー']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'トウカイテイオー']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ナイスネイチャ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ナリタブライアン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'エルコンドルパサー']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'タイキシャトル']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ヒシアマゾン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'サイレンススズカ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'シンボリルドルフ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'スペシャルウィーク']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'メジロマックイーン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ライスシャワー']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'グラスワンダー']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'サクラバクシンオー']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'カレンチャン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ミホノブルボン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'オグリキャップ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'マヤノトップガン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'セイウンスカイ']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'スーパークリーク']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'スマートファルコン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'メジロライアン']",
    "css" : {
      "background-color" : "rgb(51,255,51)"
    }
  }, {
    "selector" : "node[name = 'ナリタタイシン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'マルゼンスキー']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'キングヘイロー']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ゴールドシップ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'アグネスタキオン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ハルウララ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ビワハヤヒデ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ダイワスカーレット']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'マチカネフクキタル']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ウイニングチケット']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'エアグルーヴ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ウオッカ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'テイエムオペラオー']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'トウカイテイオー']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ナイスネイチャ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ナリタブライアン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'エルコンドルパサー']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'タイキシャトル']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ヒシアマゾン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'サイレンススズカ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'シンボリルドルフ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'スペシャルウィーク']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'メジロマックイーン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ライスシャワー']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'グラスワンダー']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'サクラバクシンオー']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'カレンチャン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'ミホノブルボン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'オグリキャップ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'マヤノトップガン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'スーパークリーク']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'セイウンスカイ']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'スマートファルコン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'メジロライアン']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node[name = 'クリスタルグリッターズ']",
    "css" : {
      "width" : 300.0
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "triangle",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "Dialog",
      "font-weight" : "normal",
      "line-color" : "rgb(132,132,132)"
    }
  }, {
    "selector" : "edge[BEND_MAP_ID > 1,109]",
    "css" : {
      "color" : "rgb(68,1,84)"
    }
  }, {
    "selector" : "edge[BEND_MAP_ID = 1,109]",
    "css" : {
      "color" : "rgb(68,2,86)"
    }
  }, {
    "selector" : "edge[BEND_MAP_ID > 1,006][BEND_MAP_ID < 1,109]",
    "css" : {
      "color" : "mapData(BEND_MAP_ID,1,006,1,109,rgb(33,145,140),rgb(68,2,86))"
    }
  }, {
    "selector" : "edge[BEND_MAP_ID > 903][BEND_MAP_ID < 1,006]",
    "css" : {
      "color" : "mapData(BEND_MAP_ID,903,1,006,rgb(251,231,35),rgb(33,145,140))"
    }
  }, {
    "selector" : "edge[BEND_MAP_ID = 903]",
    "css" : {
      "color" : "rgb(251,231,35)"
    }
  }, {
    "selector" : "edge[BEND_MAP_ID < 903]",
    "css" : {
      "color" : "rgb(253,231,37)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "BioPAX_SIF",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 0.49019607843137253,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 12,
      "border-width" : 2.0,
      "color" : "rgb(0,0,0)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 60.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(255,153,153)",
      "height" : 40.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Complex']",
    "css" : {
      "background-color" : "rgb(153,204,255)"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Complex']",
    "css" : {
      "shape" : "hexagon"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 4.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(0,0,0)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-expression-of']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'chemical-affects']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'controls-state-change-of']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'controls-phosphorylation-of']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'controls-transport-of-chemical']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'used-to-produce']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'controls-transport-of']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'consumption-controled-by']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'controls-production-of']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'catalysis-precedes']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'interacts-with']",
    "css" : {
      "line-color" : "rgb(0,85,0)",
      "target-arrow-color" : "rgb(0,85,0)",
      "source-arrow-color" : "rgb(0,85,0)"
    }
  }, {
    "selector" : "edge[interaction = 'chemical-affects']",
    "css" : {
      "line-color" : "rgb(240,144,0)",
      "target-arrow-color" : "rgb(240,144,0)",
      "source-arrow-color" : "rgb(240,144,0)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-state-change-of']",
    "css" : {
      "line-color" : "rgb(0,0,192)",
      "target-arrow-color" : "rgb(0,0,192)",
      "source-arrow-color" : "rgb(0,0,192)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-transport-of']",
    "css" : {
      "line-color" : "rgb(112,0,0)",
      "target-arrow-color" : "rgb(112,0,0)",
      "source-arrow-color" : "rgb(112,0,0)"
    }
  }, {
    "selector" : "edge[interaction = 'consumption-controled-by']",
    "css" : {
      "line-color" : "rgb(255,51,0)",
      "target-arrow-color" : "rgb(255,51,0)",
      "source-arrow-color" : "rgb(255,51,0)"
    }
  }, {
    "selector" : "edge[interaction = 'reacts-with']",
    "css" : {
      "line-color" : "rgb(0,255,0)",
      "target-arrow-color" : "rgb(0,255,0)",
      "source-arrow-color" : "rgb(0,255,0)"
    }
  }, {
    "selector" : "edge[interaction = 'neighbor-of']",
    "css" : {
      "line-color" : "rgb(0,170,0)",
      "target-arrow-color" : "rgb(0,170,0)",
      "source-arrow-color" : "rgb(0,170,0)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-expression-of']",
    "css" : {
      "line-color" : "rgb(0,160,160)",
      "target-arrow-color" : "rgb(0,160,160)",
      "source-arrow-color" : "rgb(0,160,160)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-phosphorylation-of']",
    "css" : {
      "line-color" : "rgb(0,0,255)",
      "target-arrow-color" : "rgb(0,0,255)",
      "source-arrow-color" : "rgb(0,0,255)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-transport-of-chemical']",
    "css" : {
      "line-color" : "rgb(160,0,0)",
      "target-arrow-color" : "rgb(160,0,0)",
      "source-arrow-color" : "rgb(160,0,0)"
    }
  }, {
    "selector" : "edge[interaction = 'used-to-produce']",
    "css" : {
      "line-color" : "rgb(247,85,0)",
      "target-arrow-color" : "rgb(247,85,0)",
      "source-arrow-color" : "rgb(247,85,0)"
    }
  }, {
    "selector" : "edge[interaction = 'in-complex-with']",
    "css" : {
      "line-color" : "rgb(240,0,160)",
      "target-arrow-color" : "rgb(240,0,160)",
      "source-arrow-color" : "rgb(240,0,160)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-production-of']",
    "css" : {
      "line-color" : "rgb(0,204,240)",
      "target-arrow-color" : "rgb(0,204,240)",
      "source-arrow-color" : "rgb(0,204,240)"
    }
  }, {
    "selector" : "edge[interaction = 'catalysis-precedes']",
    "css" : {
      "line-color" : "rgb(112,0,160)",
      "target-arrow-color" : "rgb(112,0,160)",
      "source-arrow-color" : "rgb(112,0,160)"
    }
  }, {
    "selector" : "edge[interaction = 'interacts-with']",
    "css" : {
      "line-color" : "rgb(0,85,0)"
    }
  }, {
    "selector" : "edge[interaction = 'chemical-affects']",
    "css" : {
      "line-color" : "rgb(240,144,0)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-state-change-of']",
    "css" : {
      "line-color" : "rgb(0,0,192)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-transport-of']",
    "css" : {
      "line-color" : "rgb(112,0,0)"
    }
  }, {
    "selector" : "edge[interaction = 'consumption-controled-by']",
    "css" : {
      "line-color" : "rgb(255,51,0)"
    }
  }, {
    "selector" : "edge[interaction = 'reacts-with']",
    "css" : {
      "line-color" : "rgb(0,255,0)"
    }
  }, {
    "selector" : "edge[interaction = 'neighbor-of']",
    "css" : {
      "line-color" : "rgb(0,170,0)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-expression-of']",
    "css" : {
      "line-color" : "rgb(0,160,160)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-phosphorylation-of']",
    "css" : {
      "line-color" : "rgb(0,0,255)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-transport-of-chemical']",
    "css" : {
      "line-color" : "rgb(160,0,0)"
    }
  }, {
    "selector" : "edge[interaction = 'used-to-produce']",
    "css" : {
      "line-color" : "rgb(247,85,0)"
    }
  }, {
    "selector" : "edge[interaction = 'in-complex-with']",
    "css" : {
      "line-color" : "rgb(240,0,160)"
    }
  }, {
    "selector" : "edge[interaction = 'controls-production-of']",
    "css" : {
      "line-color" : "rgb(0,204,240)"
    }
  }, {
    "selector" : "edge[interaction = 'catalysis-precedes']",
    "css" : {
      "line-color" : "rgb(112,0,160)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "size_rank_0_0",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 9,
      "border-width" : 0.0,
      "color" : "rgb(51,51,51)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 12.0,
      "shape" : "rectangle",
      "text-opacity" : 1.0,
      "background-color" : "rgb(204,204,255)",
      "height" : 12.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(76,76,76)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Sample1",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "Dialog",
      "font-weight" : "normal",
      "font-size" : 10,
      "border-width" : 0.0,
      "color" : "rgb(51,51,51)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 25.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(127,205,187)",
      "height" : 25.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(51,51,51)",
      "target-arrow-color" : "rgb(0,0,0)",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 1.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "Dialog",
      "font-weight" : "normal",
      "line-color" : "rgb(153,153,153)",
      "content" : "data(interaction)"
    }
  }, {
    "selector" : "edge[interaction = 'pp']",
    "css" : {
      "line-style" : "solid"
    }
  }, {
    "selector" : "edge[interaction = 'pd']",
    "css" : {
      "line-style" : "dashed"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "BioPAX",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,102,102)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 12,
      "border-width" : 2.0,
      "color" : "rgb(0,0,0)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 20.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(255,255,255)",
      "height" : 20.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Complex']",
    "css" : {
      "background-color" : "rgb(255,255,255)"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'SimplePhysicalEntity']",
    "css" : {
      "shape" : "ellipse"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Rna']",
    "css" : {
      "shape" : "ellipse"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'BiochemicalReaction']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'GeneticInteraction']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Interaction']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'TransportWithBiochemicalReaction']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Conversion']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'ComplexAssembly']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Protein']",
    "css" : {
      "shape" : "ellipse"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'RnaRegion']",
    "css" : {
      "shape" : "ellipse"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Complex']",
    "css" : {
      "shape" : "diamond"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Degradation']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Control']",
    "css" : {
      "shape" : "triangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'TemplateReactionRegulation']",
    "css" : {
      "shape" : "triangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'PhysicalEntity']",
    "css" : {
      "shape" : "ellipse"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'DnaRegion']",
    "css" : {
      "shape" : "ellipse"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'SmallMolecule']",
    "css" : {
      "shape" : "ellipse"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Dna']",
    "css" : {
      "shape" : "ellipse"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'MolecularInteraction']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'TemplateReaction']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Modulation']",
    "css" : {
      "shape" : "triangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Catalysis']",
    "css" : {
      "shape" : "triangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Transport']",
    "css" : {
      "shape" : "rectangle"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Protein-phosphorylated']",
    "css" : {
      "shape" : "ellipse"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Complex']",
    "css" : {
      "border-color" : "rgb(0,102,102)"
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'BiochemicalReaction']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'GeneticInteraction']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Interaction']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'TransportWithBiochemicalReaction']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Conversion']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'ComplexAssembly']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Complex']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Degradation']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Control']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'TemplateReactionRegulation']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'MolecularInteraction']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'TemplateReaction']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Modulation']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Catalysis']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Transport']",
    "css" : {
      "height" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'BiochemicalReaction']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'GeneticInteraction']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Interaction']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'TransportWithBiochemicalReaction']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Conversion']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'ComplexAssembly']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Complex']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Degradation']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Control']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'TemplateReactionRegulation']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'MolecularInteraction']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'TemplateReaction']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Modulation']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Catalysis']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node[BIOPAX_TYPE = 'Transport']",
    "css" : {
      "width" : 13.4
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(64,64,64)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(64,64,64)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 1.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(64,64,64)"
    }
  }, {
    "selector" : "edge[interaction = 'INHIBITION_NONCOMPETITIVE']",
    "css" : {
      "target-arrow-shape" : "tee"
    }
  }, {
    "selector" : "edge[interaction = 'INHIBITION_OTHER']",
    "css" : {
      "target-arrow-shape" : "tee"
    }
  }, {
    "selector" : "edge[interaction = 'ACTIVATION']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'INHIBITION_UNCOMPETITIVE']",
    "css" : {
      "target-arrow-shape" : "tee"
    }
  }, {
    "selector" : "edge[interaction = 'cofactor']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'ACTIVATION_ALLOSTERIC']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'right']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'INHIBITION_ALLOSTERIC']",
    "css" : {
      "target-arrow-shape" : "tee"
    }
  }, {
    "selector" : "edge[interaction = 'controlled']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'contains']",
    "css" : {
      "target-arrow-shape" : "circle"
    }
  }, {
    "selector" : "edge[interaction = 'INHIBITION']",
    "css" : {
      "target-arrow-shape" : "tee"
    }
  }, {
    "selector" : "edge[interaction = 'INHIBITION_UNKMECH']",
    "css" : {
      "target-arrow-shape" : "tee"
    }
  }, {
    "selector" : "edge[interaction = 'INHIBITION_IRREVERSIBLE']",
    "css" : {
      "target-arrow-shape" : "tee"
    }
  }, {
    "selector" : "edge[interaction = 'INHIBITION_COMPETITIVE']",
    "css" : {
      "target-arrow-shape" : "tee"
    }
  }, {
    "selector" : "edge[interaction = 'ACTIVATION_UNKMECH']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[interaction = 'ACTIVATION_NONALLOSTERIC']",
    "css" : {
      "target-arrow-shape" : "triangle"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Sample2",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(255,255,255)",
      "font-family" : "HelveticaNeue-Light",
      "font-weight" : "normal",
      "font-size" : 20,
      "border-width" : 15.0,
      "color" : "rgb(102,102,102)",
      "text-valign" : "center",
      "text-halign" : "right",
      "width" : 50.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(58,127,182)",
      "height" : 50.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 20.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(255,255,255)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Gradient1",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 8,
      "border-width" : 0.0,
      "color" : "rgb(204,204,204)",
      "text-valign" : "bottom",
      "text-halign" : "right",
      "width" : 30.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(0,0,0)",
      "height" : 30.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 1.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(102,102,102)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "size_rank_0",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 9,
      "border-width" : 0.0,
      "color" : "rgb(51,51,51)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 12.0,
      "shape" : "rectangle",
      "text-opacity" : 1.0,
      "background-color" : "rgb(204,204,255)",
      "height" : 12.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(76,76,76)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "size_rank_1",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 9,
      "border-width" : 0.0,
      "color" : "rgb(51,51,51)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 12.0,
      "shape" : "rectangle",
      "text-opacity" : 1.0,
      "background-color" : "rgb(204,204,255)",
      "height" : 12.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(76,76,76)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "default black",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,153,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 12,
      "border-width" : 0.0,
      "color" : "rgb(204,204,204)",
      "text-valign" : "bottom",
      "text-halign" : "right",
      "width" : 15.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(255,255,255)",
      "height" : 15.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "Dialog",
      "font-weight" : "normal",
      "line-color" : "rgb(0,153,0)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Minimal",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 9,
      "border-width" : 0.0,
      "color" : "rgb(51,51,51)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 42.0,
      "shape" : "rectangle",
      "text-opacity" : 1.0,
      "background-color" : "rgb(255,255,255)",
      "height" : 42.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(76,76,76)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Universe",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(0,0,0)",
      "font-family" : "Monospaced",
      "font-weight" : "normal",
      "font-size" : 18,
      "border-width" : 0.0,
      "color" : "rgb(255,255,255)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 40.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(0,0,0)",
      "height" : 40.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "dashed",
      "font-size" : 10,
      "font-family" : "Dialog",
      "font-weight" : "normal",
      "line-color" : "rgb(153,153,153)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Sample3",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(255,255,255)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 14,
      "border-width" : 8.0,
      "color" : "rgb(206,206,206)",
      "text-valign" : "bottom",
      "text-halign" : "right",
      "width" : 20.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(61,154,255)",
      "height" : 20.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(0,0,0)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(0,0,0)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "none",
      "width" : 2.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(255,255,255)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Directed",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(145,145,145)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 8,
      "border-width" : 5.0,
      "color" : "rgb(51,153,255)",
      "text-valign" : "center",
      "text-halign" : "center",
      "width" : 45.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(255,255,255)",
      "height" : 45.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,0,102)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(204,204,204)",
      "color" : "rgb(51,153,255)",
      "target-arrow-color" : "rgb(204,204,204)",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "triangle",
      "width" : 5.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 12,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(204,204,204)",
      "content" : "data(interaction)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
}, {
  "format_version" : "1.0",
  "generated_by" : "cytoscape-3.8.2",
  "target_cytoscapejs_version" : "~2.1",
  "title" : "Curved",
  "style" : [ {
    "selector" : "node",
    "css" : {
      "background-opacity" : 1.0,
      "border-opacity" : 1.0,
      "border-color" : "rgb(255,255,255)",
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "font-size" : 14,
      "border-width" : 7.0,
      "color" : "rgb(102,102,102)",
      "text-valign" : "bottom",
      "text-halign" : "right",
      "width" : 18.0,
      "shape" : "ellipse",
      "text-opacity" : 1.0,
      "background-color" : "rgb(254,196,79)",
      "height" : 18.0,
      "content" : "data(name)"
    }
  }, {
    "selector" : "node:selected",
    "css" : {
      "background-color" : "rgb(255,255,0)"
    }
  }, {
    "selector" : "edge",
    "css" : {
      "source-arrow-color" : "rgb(255,255,255)",
      "color" : "rgb(0,0,0)",
      "target-arrow-color" : "rgb(255,255,255)",
      "content" : "",
      "source-arrow-shape" : "none",
      "opacity" : 1.0,
      "target-arrow-shape" : "triangle",
      "width" : 3.0,
      "text-opacity" : 1.0,
      "line-style" : "solid",
      "font-size" : 10,
      "font-family" : "SansSerif",
      "font-weight" : "normal",
      "line-color" : "rgb(255,255,255)"
    }
  }, {
    "selector" : "edge[ id = '352' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '293' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '294' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '295' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '296' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '361' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '426' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '270' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '304' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '432' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '369' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '402' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '373' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '310' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '342' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '440' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '442' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '347' ]",
    "css" : { }
  }, {
    "selector" : "edge[ id = '351' ]",
    "css" : { }
  }, {
    "selector" : "edge:selected",
    "css" : {
      "line-color" : "rgb(255,0,0)"
    }
  } ]
} ]